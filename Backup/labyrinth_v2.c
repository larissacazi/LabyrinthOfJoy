...
Way *getPossibleWays(Edge **graph, int np, int startIdx, int *n) {
	Way* ways = NULL;
	int nWays = 0, i = 0, j = 0, old = 0, aux = 0;
	...
	while(i < nWays && i != -1) {
		...
		if(ways[i].finalized == TRUE && hasChildren(graph, ways[i].vector[ways[i].numOfPoints-1], np, fill) == TRUE) {
			printf("finalized!\n");
			reallocWay(&ways, &nWays, np, 1);
			copyLastway(&ways[nWays-1], ways[i]);
			ways[nWays-1].finalized = FALSE;
			from = ways[nWays-1].vector[ways[nWays-1].numOfPoints-1];
			printWays(ways, nWays);
		...
		if(num > 1) {
			...
			printf("aux = %d\n", aux);
			while(aux < nWays) {
					if(ways[aux].vector[ways[aux].numOfPoints-1] == from) {
							printf("Ok aux [%d] from[%d] to[%d]\n", aux, from, to);
							printf("nextChildren :");
							nextChildren = getNextChildren(graph, np, from, to, fill);
							printf("nextChildren = [%d]\n", nextChildren);
							to = nextChildren;
							printf("From[%d] To[%d]\n", from, to);
							insertPointInWay(&ways[aux], graph, from, to);
							fill[to] = 1;
					}
					aux++;
			}
